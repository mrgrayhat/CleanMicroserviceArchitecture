...
namespace StorageManagement.Application.Events
{
    ...
    public class ContentRequestedEmailSenderHandler : INotificationHandler<ContentRequestedEvent>
    {
        ...
        {
            // IMessageSender.Send("Storage content with id {ContentId} requested at {RequestDate} from client {Client}", notification.ContentId, notification.RequestDate, notification.Client);
            return Task.CompletedTask;
        }
    }
    ...
    public class PostRequestedLogSenderHandler : INotificationHandler<ContentRequestedEvent>
    {
        ...
        public Task Handle(ContentRequestedEvent notification, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Storage content with id {ContentId} requested at {RequestDate} from client {Client}", notification.ContentId, notification.RequestDate, notification.Client);
            return Task.CompletedTask;
        }
    }
}
