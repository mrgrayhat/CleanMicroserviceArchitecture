...
namespace SaeedRezayi.Services.Account
{
    public class TokenFactoryService : ITokenFactoryService
    {
        ...
        {
            var refreshTokenSerial = _securityService.
                CreateCryptographicallySecureGuid().ToString().Replace("-", "");
        ...
        private async Task<(string AccessToken, IEnumerable<Claim> Claims)> createAccessTokenAsync(UserInfo user)
        {
            var claims = new List<Claim>
            {
                // Unique Id for all Jwt tokes
            ...
            // add roles
            List<RoleInfo> roles = await _rolesService.FindUserRolesAsync(user.Id);
